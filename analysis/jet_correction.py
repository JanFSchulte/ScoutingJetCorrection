...
def derive_correction(table, pt_bins=None, eta_bins=None,
    ...
    scouting pt-floor mismatch applies here (drops response entries with
    pt_field below this, default 10 GeV -- deliberately below offline's own
    15 GeV floor, so genuine soft scouting jets this correction needs to
    learn from aren't excluded). Pass None to disable.
    """
    if pt_bins is None:
...
def derive_response_curve(table, pt_bins=None, eta_bins=None,
    ...
    binning -- deliberately LOWER than the hard floor MiniAOD/NanoAOD
    enforces on the OFFLINE side (off_pt.min() is exactly 15.0 in every
    comparison table checked, MC and data alike): since this curve gets
    INVERTED to correct a raw scouting jet's pt up toward the true/offline
    scale, and the response here is well below 1 at low pt, a genuine raw
    scouting jet at 10-14 GeV can legitimately correct up past 15 GeV --
    excluding it from the derivation would mean never learning how to
    correct exactly the jets that most need it. The 10 GeV default excludes
    only the worst, most extreme tail: dr_match_jets() has no response/pt-
    consistency requirement, so a real >=15 GeV offline jet can end up
    geometrically "matched" to an unrelated near-zero-pt scouting noise
    candidate whenever the genuine corresponding scouting jet wasn't
    reconstructed (a matching/efficiency failure, not a pt-response
    difference this multiplicative correction should be fit to). Checked
    empirically (see project notes): dR to the matched offline jet degrades
    smoothly as scout_pt drops from ~15 GeV to 0 -- there is no sharp
    "noise vs. real" elbow, so 10 GeV is a judgment call trading off "keep
    genuinely soft, hard-to-reconstruct real jets" against "don't let the
    most poorly-measured tail dominate the median" -- not a uniquely correct
    number. Pass min_scout_pt=None to disable, or 15.0 for the more
    conservative (offline-floor-matching) cut this replaced."""
    if pt_bins is None:
        pt_bins = _default_pt_bins()
    if eta_bins is None:
        eta_bins = _default_eta_bins()
